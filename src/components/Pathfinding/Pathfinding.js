    ...
    dijkshtra=(e)=>{
        ...
        const animatePath=()=>{
            if(j==shortestPath.length){
                // Calculate actual path cost for A*
                let pathCost = 0;
                if (this.state.method === "A* Search") {
                    for (let k = 1; k < shortestPath.length; k++) {
                        const prev = shortestPath[k-1];
                        const curr = shortestPath[k];
                        const isDiagonal = Math.abs(curr.row - prev.row) + Math.abs(curr.col - prev.col) === 2;
                        pathCost += isDiagonal ? Math.SQRT2 : 1;
                    }
                } else {
                    pathCost = shortestPath.length - 1; // For Dijkstra's, count moves (nodes - 1)
                }
